...
import { useSubmitSafetyLogSignature } from "@/hooks/useSubmitSafetyLogSignature";

export default function SafetyLogSignPage() {
  ...
  const submitSignatureMutation = useSubmitSafetyLogSignature();
  ...
  const handleSubmit = () => {
    if (!safetyLogId || !siteId || !user?.employeeId) {
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen flex-col bg-worker-neutral-50">
        <Header title="안전교육일지 서명" showBackButton />
